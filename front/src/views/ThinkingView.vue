<template>
  ...
  <body style="justify-content: center;">
    ...
    <div>
      <ThinkingLayout v-for="advice in content" :key="advice.id" :content="advice" :privileges="this.privileges"
...
<script>
import ThinkingLayout from '@/components/ThinkingLayout.vue';
...
export default {
  ...
  methods: {
    ...
    modify_form(id) {
      ...
      axios.post("http://127.0.0.1:8000/RaicesDigitales/" + this.page + "/get", { lan: "ES" }).then((result) => {
        ...
        if (result.status == 200) {
          es_array = result.data.thinking.content;
      ...
      axios.post("http://127.0.0.1:8000/RaicesDigitales/" + this.page + "/get", { lan: "EN" }).then((result) => {
        if (result.status == 200) {
          en_array = result.data.thinking.content;
    ...
    delete(id) {
      ...
      axios.post("http://127.0.0.1:8000/RaicesDigitales/" + this.page + "/get", { lan: "ES" }).then((result) => {
        console.log(result)
        if (result.status == 200) {
          let es_array = result.data.thinking.content;
      ...
      axios.post("http://127.0.0.1:8000/RaicesDigitales/" + this.page + "/get", { lan: "EN" }).then((result) => {
        if (result.status == 200) {
          let en_array = result.data.thinking.content;
  ...
  created() {
    axios.post("http://127.0.0.1:8000/RaicesDigitales/thinking/get", this.lan).then((result) => {
      console.log(result);
      if (result.status == 200) {
        this.error_handler = false
        this.main_data = result.data.thinking.main_data;
        this.content = result.data.thinking.content;
      }
    }).catch(function (error) {
      console.log(error)
    })
